...
<head>
    ...
    {{-- endpoint untuk sync preferensi ke backend (dipakai currency-lang.js) --}}
    <meta name="pref-url" content="{{ route('pref.update') }}">

    <title>@yield('title', config('app.name', 'GLOOMIE SUNDAY - UNUSUAL CLUB'))</title>

    {{-- Prefs (locale/currency) seed untuk JS --}}
    ...
    {{-- Cache-busting for public assets --}}
    @php
...
<body>
    ...
    {{-- FAB: Locale & Currency (kiri-bawah) — include SEKALI --}}
    ...
    {{-- App JS (public/) --}}
    <script src="{{ asset('js/currency-lang.js') }}?v={{ $v_cur }}" defer></script>
